...
import com.cyberwolf.musicplayer.dialogs.AddPlaylistDialog;
...
import com.cyberwolf.musicplayer.widgets.BubbleTextGetter;
import com.cyberwolf.musicplayer.widgets.MusicVisualizer;
...
public class SongsListAdapter extends RecyclerView.Adapter<SongsListAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public ItemHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        if (isPlaylist) {
            ...
            ItemHolder ml = new ItemHolder(v);
           // ml.albumArt.setImageResource(R.drawable.fireskullsmall);
            return ml;
        }
    }
    ...
    public void onBindViewHolder(ItemHolder itemHolder, int i) {
        ...
        itemHolder.albumArt.setImageResource(R.drawable.fireskullsmall);

        ImageLoader.getInstance().displayImage(TimberUtils.getAlbumArtUri(localItem.albumId).toString(), itemHolder.albumArt, new DisplayImageOptions.Builder().cacheInMemory(true).build()); //resetViewBeforeLoading was true but i set the default img resource LOLOL
        ...
        if(!isPlaylist) {
            if (MusicPlayer.getCurrentAudioId() == localItem.id) {
                ...
                if (MusicPlayer.isPlaying()) {
                   ...
                   // itemHolder.playingif.setAlpha(0.0f);
                   // itemHolder.playingif.setVisibility(View.VISIBLE);
                   // itemHolder.playingif.animate().alpha(1.0f).setDuration(1500);
                    itemHolder.visualizer.setVisibility(View.INVISIBLE);
                }
            } else {
                if (isPlaylist)
                    itemHolder.title.setTextColor(Color.WHITE);
                else
                    // itemHolder.title.setTextColor(Color.LTGRAY);
                   // itemHolder.playingif.setVisibility(View.GONE);
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
       ...
       // TextView playingif;
        ...
        public ItemHolder(View view) {
          ...
          //  playingif = (TextView)view.findViewById(R.id.gifTextView);
