...
package com.cyberwolf.musicplayer.subfragments;
...
import com.cyberwolf.musicplayer.activities.BaseActivity;
import com.cyberwolf.musicplayer.listeners.MusicStateListener;
...
import com.cyberwolf.musicplayer.utils.SlideTrackSwitcher;
import com.cyberwolf.musicplayer.utils.TimberUtils;
import com.cyberwolf.musicplayer.widgets.PlayPauseButton;
...
public class QuickControlsFragment extends Fragment implements MusicStateListener {
    ...
    public void updateNowplayingCard() {
        ...
        if (!duetoplaypause) {
            mTitle.setTranslationX(100);
            mTitle.setAlpha(0.0f);
            mTitle.animate().translationX(0).alpha(1.0f).setDuration(2500);

            mArtist.setTranslationX(100);
            mArtist.setAlpha(0.0f);
            mArtist.animate().translationX(0).alpha(1.0f).setDuration(3000);

            mTitleExpanded.setTranslationY(50);
            mTitleExpanded.setAlpha(0.0f);
            mTitleExpanded.animate().translationY(0).alpha(1.0f).setDuration(1500);

            mArtistExpanded.setTranslationY(50);
            mArtistExpanded.setAlpha(0.0f);
            mArtistExpanded.animate().translationY(0).alpha(1.0f).setDuration(2200);
            ...
            ImageLoader.getInstance().displayImage(TimberUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), mAlbumArt,
                    new DisplayImageOptions.Builder().cacheInMemory(true)
                        ...
                        public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                            Bitmap failedBitmap = ImageLoader.getInstance().loadImageSync("drawable://" + R.drawable.fireskullsmall);
                            if (getActivity() != null)
